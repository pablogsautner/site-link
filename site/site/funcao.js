document.addEventListener('DOMContentLoaded', function () {
  ...
  function createNewLink() {
    if (linkCount < maxLinks) {
      ...
      // Obtém o nome, o URL e a cor de fundo dos campos de entrada
      const linkName = linkNameInput.value;
      const linkURL = linkURLInput.value;
      const linkBackgroundColor = document.getElementById(
        'linkBackgroundColor'
      ).value;
      ...
      if (linkName && linkURL) {
        ...
        // Aplica a cor de fundo ao <a> (link)
        newLink.style.backgroundColor = linkBackgroundColor;

        // Adiciona o link ao <li>
        newLinkItem.appendChild(newLink);

        // Adiciona o <li> à lista de links em "Meus Links" (container-right)
        const myLinksList = document.getElementById('myLinks');
        myLinksList.appendChild(newLinkItem);

        // Cria um novo elemento <li> para o link na lista de "Links Criados" (container-left)
        const createdLinksList = document.querySelector('.created-links');
        const newLinkItemForCreated = document.createElement('li');
        const newLinkForCreated = document.createElement('a');
        newLinkForCreated.href = linkURL;
        newLinkForCreated.textContent = linkName;

        // Aplica a cor de fundo ao <a> (link) em "Links Criados"
        newLinkForCreated.style.backgroundColor = linkBackgroundColor;

        newLinkItemForCreated.appendChild(newLinkForCreated);
        createdLinksList.appendChild(newLinkItemForCreated);
